...
```xml
<ItemGroup>
    <COMFileReference Include="$(SomePath)RengaCOMAPI.tlb"/
</ItemGroup>
```
...
```
**Примечание**: данная конструкция сработает только для языка программирования Visual C++ (та версия языка от Microsoft). Обычный C++ "не знает" такой команды.
...
Подключите их в свой проект. Ниже пример подключения для случая Qt в файле `pro`
