...
Следует открывать объект в том режиме, для которого нужен соответствующий доступ. Несмотря на удобство режима Write для чтения и записи, при его использовании затрачивается больше ресурсов на стороне нативного кода. Если вы не уверены, что открываемый объект : это тот, с которым вы хотите работать, откройте его сперва в режиме чтения, а затем переведите объект из режима чтения в режим записи. Дополнительные сведения об изменении способа открытия объекта см. [в разделе](./Topic_CreateAndEditAcadObjects_EditOpeningMode.md). Обе функции **GetObject** и **Open** возвращают объект (подробнее о каждой см. [раздел](./Topic_CreateAndEditAcadObjects_OpenCloseWithoutTr.md)). При разработке на C# вам необходимо будет дополнительно приводить тип возвращаемого объекта к нужному классу с помощью модификатора **as**. 
