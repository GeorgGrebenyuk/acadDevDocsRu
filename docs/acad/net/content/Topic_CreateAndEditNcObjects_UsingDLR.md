Существовавшая в AutoCAD .NET API динамическая типизация, позволявшая получать прямой доступ к объектам без их открытия для чтения, в настоящем nanoCAD .NET API не реализована. Использовать динамическую типизацию целесобразно использовать только для работы с COM-интерфейсами - в этом случае нет опасений обратиться к иной версии COM-библиотеки (так как версии nanoCAD Type Library меняются для каждого основного релиза).
